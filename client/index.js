...
let canPlayMissSound = true;
const missSound = new Audio("/public/audio/wrong.mp3");

const guessInput = document.getElementById("guess-input");
const feedback = document.getElementById("feedback");

function updateCatchCounter() {
  const caughtCount = guessedPokemonIds.length;
  ...
  if (caughtEl && totalEl) {
    ...
    caughtEl.classList.remove("animate");
    void caughtEl.offsetWidth;
    caughtEl.classList.add("animate");
    ...
    if (pokeball) {
      ...
      setTimeout(() => {
        pokeball.classList.remove("spin-once");
      }, 600); // gelijk aan de duur van je rotate360 animatie
    }
  }
}
...
function applyPokedexFilter() {
  ...
  document
    ...
    .forEach((listItem) => {
      const id = Number(listItem.dataset.id);
      const shouldShow = !showOnlyGuessed || guessedPokemonIds.includes(id);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  guessInput.addEventListener("input", (e) => {
    ...
    const matchingPokemons = allPokemons.filter(p =>
      !guessedPokemonIds.includes(p.id) &&
  ...
  guessForm.addEventListener("submit", (e) => {
    ...
    if (guess === currentPokemon.name.toLowerCase()) {
      ...
      localStorage.setItem(
        "guessedPokemonIds",
        JSON.stringify(guessedPokemonIds)
      );
      ...
      lastCorrectContainer.style.display = "block";
      ...
      if (canPlayMissSound) {
        canPlayMissSound = false;
        missSound.currentTime = 0;
        missSound.play().catch(err => {
          console.warn("Kon foutgeluid niet afspelen:", err);
        });

        setTimeout(() => {
          canPlayMissSound = true;
        }, 700);
      }
...
const closeBtn = document.getElementById("close-pokemon-list");

if (closeBtn && pokemonList) {
...
function renderPage(page) {
  let visibleItems = allPokemonItems.filter(pokemon => {
    ...
    const isGuessed = guessedPokemonIds.includes(id);
